...
//            diagonals = true;
    ...
    private static boolean checkDiagonals(int[][] array, int magicConstant) {

        boolean isDiagonal = false;

        int numberOfRows = array.length;
        int numberOfCols = array[0].length;
        int sumOfMajorDiagonal = 0;
        int sumOfMinorDiagonal = 0;

        // check '\' diagonal (major diagonal)
        int colIndex = 0;
        for(int i=0; i<numberOfRows; i++) {
            sumOfMajorDiagonal += array[i][colIndex];
            colIndex++;
        }

        // check '/' diagonal (minor diagonal)
        colIndex = numberOfCols-1;
        for(int i=0; i<numberOfCols; i++) {
            sumOfMinorDiagonal += array[i][colIndex];
            colIndex--;
        }

        if( sumOfMajorDiagonal==magicConstant && sumOfMinorDiagonal==magicConstant )
            isDiagonal=true;
        else
            System.out.println("There were invalid diagonals.");
